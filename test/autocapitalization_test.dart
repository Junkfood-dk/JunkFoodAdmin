...
void main() {
  testWidgets('Auto-capitalization for title and description fields', (WidgetTester tester) async {
    ...
    {'input': 'this is a test. this is another test.', 'expected': 'This is a test. This is another test.'},
    {'input': 'this ends with space. ', 'expected': 'This ends with space. '},
    {'input': 'hello! how are you? i\'m fine.', 'expected': 'Hello! How are you? I\'m fine.'},
    {'input': 'section 3.2 starts here: the test.', 'expected': 'Section 3.2 starts here: the test.'},
    {'input': 'détente. œuvre. naïve. café.', 'expected': 'Détente. Œuvre. Naïve. Café.'},
    {'input': '123 times are fun times. okay?', 'expected': '123 times are fun times. Okay?'},
    {'input': 'æblegrød er godt. økonomi er vigtig. Århus er smuk.', 'expected': 'Æblegrød er godt. Økonomi er vigtig. Århus er smuk.'},
    {'input': '', 'expected': ''},
  ];
    ...
    for (var testCase in titleFieldTestCases) {
      ...
      await tester.pump(const Duration(milliseconds: 100)); // to avoid timeout
      expect((tester.widget(titleFieldFinder) as TextFormField).controller?.text, equals(testCase['expected']));
    }

    for (var testCase in descriptionFieldTestCases) {
      await tester.enterText(descriptionFieldFinder, testCase['input']!);
      await tester.pump(const Duration(milliseconds: 100)); // to avoid timeout
      expect((tester.widget(descriptionFieldFinder) as TextFormField).controller?.text, equals(testCase['expected']));
    }
  });
}
